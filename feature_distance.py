...
def calculate_feature_distance(args):
    ...
    for i, (x, y) in enumerate(dataloader): 
        ...
        cosine_similarity = F.cosine_similarity(ten_features[0], ten_features[5], dim=1)
        cosine_similarities += [val.item() for val in cosine_similarity]

        # append all features labeled as (digit, hue) to a list
        for digit, feature in zip(y, ten_features):
            extracted_features.append((digit.item(), hue, feature))
    ...
    for i, (digit, hue, _) in enumerate(extracted_features):
        ...
        ax.scatter(pca_features[i, 0], pca_features[i, 1], c=hue, marker=digit)
