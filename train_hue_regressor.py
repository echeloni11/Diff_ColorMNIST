...
train_dataloader = Data.DataLoader(dataset=train_dataset, batch_size=256, shuffle=True)
test_dataloader = Data.DataLoader(dataset=test_dataset, batch_size=256, shuffle=True)

model = Classifier(output_dim=1).to(device)

losses_1 = []
losses_2 = []


def train(model, device, train_loader, optimizer, epoch, p_unif=1, noisy=False):
    model.train()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = F.mse_loss(output.view(-1), hues.view(-1))
        # do a backward pass
        loss.backward()
        # update the weights
        optimizer.step()
...
accuracy = []
avg_loss = []
def test(model, device, test_loader, p_unif=1):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # pass the image to the model and assign the output to variable named output
            output = model(data)
            test_loss += F.mse_loss(output.view(-1), hues.view(-1), reduction='sum').item() # sum up batch loss
          
            # pred = output.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
            correct += (torch.abs(output.view(-1)-hues.view(-1)) < 0.05).sum().item()
...
for epoch in range(0, 61):
    ...
    train(model, device, train_dataloader, optimizer, epoch, noisy=noisy)
    test(model, device, test_dataloader)
    if epoch % 20 == 0:
        if noisy:
            name = f"./trained_classifiers/hue_noisy_{epoch}.pt"
        else:
            name = f"./trained_classifiers/hue_clean_{epoch}.pt"
